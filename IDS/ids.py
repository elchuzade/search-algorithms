...
def combiner(info_carrier, next_move, ij,
             ...
             previous_expand_index, current_state, depth):
    info_carrier = []
...
def solution_state_index(solution_index, state_space, expand):
    tracker = 0
    next_to_check = 0
    i = 1
    last_state = state_space.index(expand)
    solution_index.append(last_state)
    while i != 0:
        prev_step = solution_index[tracker]
        state_to_add = state_space[prev_step]
        state_last_digit = state_to_add[-2]
...
solution_index = solution_state_index(solution_index, state_space, expand)
...
print('total state space -', len(state_space))
